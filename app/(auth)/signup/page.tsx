    ...
    return (
      <div className={`h-screen flex items-center justify-center bg-white rounded-3xl border border-border transition-opacity duration-300 ${exiting ? 'opacity-0' : 'opacity-100'}`}>
        <div className="w-full max-w-md flex flex-col gap-8 px-6 py-8 sm:pt-32 sm:pb-8 sm:px-0">
            ...
            <div className="flex gap-2 sm:gap-3 justify-center">
              {otp.map((digit, index) => (
                <InputField
                  ...
                  className={`w-12 h-12 sm:w-14 sm:h-14 p-0 text-center text-xl sm:text-2xl font-semibold rounded-xl sm:rounded-2xl border transition-all ${otpError ? 'border-red' : 'border-border focus:border-accent'} ${digit ? 'text-dark' : 'text-dark-50'}`}
                />
              ))}
            </div>
          ...
          <button onClick={() => { setShowOtp(false); setOtp(['', '', '', '', '', '']); setOtpError(''); setResendTimer(0); }} className="text-sm text-dark-70 hover:text-dark text-center">
            Change email
          </button>
        </div>
      </div>
  ...
  return (
    <div className="h-screen flex items-center justify-center bg-white rounded-3xl border border-border">
      <div className="w-full max-w-md flex flex-col gap-8 px-6 py-8 sm:pt-32 sm:pb-8 sm:px-0">
        ...
        <p className="text-center text-sm text-dark-50 mt-24">
